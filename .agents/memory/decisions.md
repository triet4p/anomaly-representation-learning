...
## [2026-09-12] Bind Sprint 16 C5 to the accepted V2 counterfactual objective

**Decision:** Treat masked prediction, EMA target construction, masking schedules, and contrastive weighting as NOT PRESENT in the accepted V2 checkpoints (never blamed, never verdict candidates); diagnose only the real CounterfactualCriterion terms (boundary-margin, background, variance, covariance) with exactly two FIT-healthy-only retrains (background=0, covariance=0, control only, ≤300 steps each).
**Alternatives considered:** Ratify protocol v3's C5 variants (mask ratio, contrastive weight) and run them; invent V2-native multi-factor retrains on both checkpoints; or skip retrains and rely on frozen diagnostics alone.
**Reason:** Signature/config/path inspection proves the frozen v3 knobs do not exist in the accepted stack, so running them is impossible and substituting unapproved variants would be outcome-shaping; the two single-factor zeroings are the smallest executable causal tests of objective pressure within the frozen ≤2-run budget, with hybrid replication explicitly unfunded and capped at SUSPECT-class.
**Consequences:** Protocol v5 carries the C5 re-binding; retrain mode fails closed on any other variant/checkpoint; a C5 BOTTLENECK claim additionally needs hybrid replication via a new ruling; future agents must not reintroduce masked/contrastive blame without explicit instruction and new accepted evidence.

## [2026-09-12] Freeze Task 11 geometry ladder with fixed mean reduction and batched-exact production mirrors

**Decision:** Measure Task 11 as a five-arm ladder per input family (R-simple global-centroid Euclidean; G-b collapsed-conditioning global Mahalanobis; G-hier refit single-resolve; G-mix refit mixture; G-prod restored-bank mixture, learned families only) with one frozen per-file mean reduction for every arm, evaluating shipped per-patch loops through batched-exact mirrors proven equal to production methods in --self-test.
**Alternatives considered:** Vary the file reduction per arm (e.g. production aggregate_file_state for G-prod); call production per-patch Python loops directly at full Confirmation scale; fit refit banks on standardized instead of raw family space; score oracle through a checkpoint bank.
**Reason:** Varying reduction would confound geometry readability with aggregation (Task 12's boundary); direct production loops cost ~10x runtime at identical numerics (self-test proves equality to 1e-3); production fits references on raw latents so refits must too; no oracle-dim bank exists in any accepted checkpoint, forcing honest NOT_RUN over a fabricated cross-dimensional score.
**Consequences:** Task 12 owns all aggregation/scorer variation and must not re-litigate the mean-reduction hold-fixed choice; future geometry diagnostics reuse the batched mirrors only with the self-test equality gate intact; G-prod conclusions stay scoped to patch-level bank readability, not shipped file scores.

## [2026-09-12] Scope Task 12 S_pred arms to in-vocabulary files, never remap

**Decision:** Run Task 12 S_pred (head-dependent) arms only on files whose training-time conditioning indices fall inside the checkpoint embedding vocabulary (robot < n_robots and program < n_programs); out-of-vocabulary files get NaN head signals, are counted per history, and are never remapped, while S_pop arms keep full-universe support via the Task 11 fallback rule.
**Alternatives considered:** Remap out-of-range indices into vocabulary; crash the run and declare Task 12 unexecutable; drop the S_pred family and test aggregation on S_pop only.
**Reason:** The production head asserts (device-side) on out-of-vocabulary indices and the checkpoint vocabulary (3x3) covers only ~22% of the 9x8 evaluation population, so full-universe S_pred scoring is impossible without fabricating conditioning; remapping would invent semantics the checkpoint never fitted, while the Task 9 v5 skip-rule precedent (counted skips, never remapped) preserves honesty and keeps identical support within each family.
**Consequences:** Task 12 S_pred conclusions carry reduced-support caveats with per-history vocabulary counts; the vocabulary shortfall itself is reported as a scorer-readability finding; future scorer work must not silently remap conditioning indices without explicit instruction.

## [2026-09-12] Assemble Task 13 matrix by reuse plus one local X7 execution

**Decision:** Execute only the missing X7 cross-combination (oracle patch features through production-form tail reduction) as a local CPU-only run on canonical roots, and assemble Task 13 matrix cells X1-X6/X8 by re-tallying (never recomputing) the accepted Tasks 4/7/8/11/12 artifacts, with cross-row absolutes never arithmetically mixed and each row's delta kept on its own identical support.
**Alternatives considered:** Re-execute all eight cells remotely for uniform provenance; run X7 on the GPU server loop; or report X7 as NOT_RUN and leave the matrix incomplete.
**Reason:** Seven cells already hold accepted identical-support comparisons and recomputation would manufacture new outcomes without new information (plus double-credit risk flagged by protocol v3 section 7); X7 needs no checkpoint or GPU, so the local canonical form is the cheapest honest execution, and its mean arm reproducing Task 11 oracle cells cell-for-cell proves cross-task consistency.
**Consequences:** Task 13 claims new evidence only for X7; future substitution work must preserve the no-recompute/no-double-credit rule for accepted cells and keep cross-row methodological differences (file- vs patch-level) out of arithmetic.

## [2026-09-12] Execute Task 14 C2/C6/C7/C8/C9 variants; cite C5, cap C1/C3/C4

**Decision:** Execute 13 new §8 intervention counts (C2:3 local CPU, C6:3, C7:2, C8:2, C9:3 server) with Task 14-owned execution records even where variant forms coincide with accepted M-task measurements (deterministic reproduction expected); cite Task 9 one-shots for C5 without re-execution; resolve C1 UNRESOLVED/NOT PRESENT, C3 SUSPECT-capped with no bypass, C4 NOT_RUN for lack of localization evidence.
**Alternatives considered:** Cite accepted M-task numbers as the §8 executions (no new runs); re-run C5 retrains; run C4 bypasses despite null localization; run C1 N-variants as causal evidence.
**Reason:** Section 9.1 requires every predeclared variant to execute once under Task 14's counted scope and section 13 forbids double-credit, so citation alone fails compliance while deterministic reproduction preserves consistency; C5 reruns are explicitly forbidden by the v5 one-shot rule with budget exhausted; C4/C1 executions would be unmotivated fishing or non-causal by logged decision.
**Consequences:** Task 16 movement/restoration arithmetic consumes Task 14 deltas (not M-task numbers) for C2/C6/C7/C8/C9; C5 verdict input stays at Task 9's SUSPECT ceiling; 7 of 20 counts remain unspent; future agents must not re-execute C5 or claim C1/C3/C4 causality from these records.

## [2026-09-12] Freeze Task 15 C7 replication rule R1-R5; nuisance needs server

**Decision:** Replicate only C7 G-a/G-b (the sole Task 14 recovery direction) under frozen rule R1-R5 (P/W-macro recovery ≥3/4 histories each, post LCB>0.55 per counted cell, nuisance exceedance/FAR/ΔFAR veto-signal, descriptive severity, A/threshold-edge cells never count); execute only the missing R3 nuisance cells via a slim server driver reusing Task 14 event scores, with C2/C6/C8/C9 documented as non-candidates.
...
## [2026-09-13] Open Sprint 17 as a controlled diagnostic ablation

**Decision:** Open Sprint 17 under explicit user authorization to compare the unchanged architecture baseline against two prospectively registered alternatives for each provisional SUSPECT C2/C3/C5/C6/C7/C8/C9, followed by bounded adjacent-pair and stack combinations, using fresh disjoint roles generated by the unchanged accepted Sprint 15 Candidate 7 methodology.
**Alternatives considered:** Preserve the prior empty Sprint 17 scope; ship a C7 bank rewrite as if Sprint 16 had proven it; run an unconstrained architecture/hyperparameter search; or select and confirm alternatives on the already-consumed Sprint 15 Confirmation histories.
**Reason:** Sprint 16 localized seven provisional leads but established zero replicated bottlenecks, while the user explicitly authorized a new ablation rather than a production fix. A frozen baseline/single/combination matrix on fresh Development and untouched Confirmation roles tests those hypotheses without rewriting Sprint 16 history, adaptively reusing outcomes, or turning ambiguous attribution into a causal claim.
**Consequences:** `docs/sprint-plans/sprint-17.md` is the active plan; C1/C4 remain fixed, all single arms change one principal mechanism, combination membership is bounded and locked before Confirmation, the Sprint 15 generator method and Sealed prohibition remain unchanged, `S_pred`/`S_pop` remain separate, and no ablation result is production authorization without later explicit planning and gates.

## [2026-09-13] Restart Sprint 17 prospectively with protocol v3

**Decision:** Preserve protocol v2 and its 15/16 preflight failure as immutable history, then restart once under protocol v3 with exactly one entirely new 16-role seed block frozen before preflight; keep the Sprint 15 Candidate 7 generator method, quotas, gates, ablation registry, and Sealed prohibition unchanged, and stop the current execution after Task 6 before GPU-dependent Task 7.
**Alternatives considered:** Close Sprint 17 at v2 as `BLOCKED / UNRESOLVED`; replace only failing seed 2709; or prospectively screen multiple reserve blocks until one passes.
**Reason:** The user explicitly chose a v3 restart. Replacing only the observed failing seed or screening reserve blocks would turn preflight into favorable seed selection; abandoning the complete v2 roster and freezing one all-new block preserves a one-shot prospective comparison while retaining the accepted Sprint 15 data-generating method.
**Consequences:** Tasks 2A–2C must freeze and evidence-review v3 before Task 3 can resume; v2 files and blocker evidence remain authoritative history; no v2 seed may be reused; any v3 feasibility miss stops again without replacement; Tasks 7+ and GPU-server work remain pending.

## [2026-09-14] Waive the exact Sprint 17 Task 4 Design cohort-mix miss

**Decision:** Under explicit user authorization, bypass only `H-S17-V3-DESIGN-03 / cohort_mix_15_60 / P=91/149=0.6107382550 > 0.60` through protocol v4, preserve the original protocol-v3 `NOT_MEASURABLE` evidence, and require the qualified state `MEASURABLE_WITH_USER_WAIVER` before allowing Task 5.
**Alternatives considered:** Stop Sprint 17 at Task 4; regenerate or replace the failing history; relax the 0.60 limit globally; omit Design from certification; or treat the original gate as passed.
**Reason:** The user explicitly overrode this blocker. A single named post-observation waiver is the narrowest implementation: the failure is Design-only and marginal, while Development and Confirmation pass every original structural and observable gate. Preserving the failure and qualification avoids rewriting it as prospective evidence or weakening unrelated gates.
**Consequences:** Protocol v4 must change no root, seed, generator, arm, metric, Development/Confirmation gate, or Sealed rule; Tasks 4A–4B must pass evidence review before Task 4/5 advance; every Sprint 17 result must disclose the waiver and may not claim unqualified `MEASURABLE`; the execution still stops before Task 7/GPU.
