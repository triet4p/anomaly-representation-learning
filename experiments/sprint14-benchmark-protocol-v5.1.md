# Sprint 14 Benchmark Recovery Protocol v5.1 (FROZEN — MAIN-AUTHORIZED SINGLE RE-EVALUATION)

**Status:** Internally frozen by the Sprint 14 worker as a tooling-only correction. Main directly authorizes exactly one deterministic re-evaluation under this version as specified in §9/§13; any further execution (including any Task 8 materialization) requires a fresh evidence review first. Any semantic DGP change requires a new protocol version (`v5.2`, …), Main approval, and a fresh evidence review before materialization.
...
**Protocol identifier:** `sprint14-benchmark-protocol-v5.1`. This file's own SHA256 digest is recorded after freezing in the cycle ledger (self-embedding a digest would be circular).

**Diagnostic cycle:** cycle 3 of at most 3 (Cycles 1–2 consumed; no cycles remain after this one).

## 0. Amendment record (v5 → v5.1; tooling only, zero DGP change)

- `experiments/sprint14-benchmark-protocol-v1.md` (`8279e2b5…6c8b6697a1b`), `-v2.md` (`5b6eb212…7a7c67404`), `-v3.md` (`45b5ef69…fb85d1`), `-v4.md` (`88f2b426…bb3176`), `-v4.1.md` (`66572d72…03a5ebb4`), `-v5.md` (`159225d7…01d3016`) preserved byte-identical. Cycle 1 roots/audits retired (`e018`); Cycle 2 roots preserved as diagnostic-only; quarantined Cycle 1 numbers are not cited anywhere in this file.
- Exact deltas, nothing else:
  1. **Task 7 share-gate removal (§13):** the non-normative raw-sample ±8pp gate (which lived only in the proof script, never in any protocol or exit gate) is removed. In its place the harness asserts the configured cohort probabilities exactly (`P=.45/W=.30/A=.25` from the frozen resolved configuration) and reports raw and evaluable realized shares descriptively without gating.
  2. **Re-evaluation mode (§§9, 13):** a `--reevaluate` mode reads the exact preserved seed-796 roots after verifying their recorded manifest/shard hashes; it never regenerates, mutates, or overwrites any root, and writes all outputs to the new collision-free evidence path `artifacts/sprint-14/server-task7-cycle-3-reeval/`.
  3. **Provenance-gate carry-forward (§9):** the R1–R4 formulation stands unchanged.
- The v5 DGP (A+C settings, routes, physics, semantics, floors, probe, roster) is otherwise carried forward byte-for-meaning identical. No Task 7 rerun from scratch (no new generation) occurs under v5.1 — only the deterministic re-evaluation of preserved roots specified here.

## 1. Cycle-3 DGP (identical to v5; frozen settings restated)
...
- One bounded DGP amendment per diagnostic cycle (this v5/v5.1 line is the cycle-3 amendment), driven only by design-role structural diagnostics and the frozen probe. Learned representation, anomaly, warning, and calibrated-risk outputs cannot select generator settings, histories, thresholds, or gates.
...
- Canonical commit, runtime versions, output roots, and seal format are recorded in the cycle ledger at materialization time (Task 8), not invented here. The exact canonical implementation commit for v5 execution is established at Task 6 and independently evidence-reviewed with Task 7 before Task 8; reviewer approval of the v5.1 tooling correction governs the single deterministic re-evaluation specified below.
...
- **Tooling roots (collision-free):** Task 7 proof entry points write only to their frozen output roots (`server-task7-cycle-3/`, `server-task7-cycle-2-rerun/`) with disposable data roots under `data/generated/sprint14-task7-c3(-rerun)/`, and the v5.1 re-evaluation writes only to `artifacts/sprint-14/server-task7-cycle-3-reeval/` (disposable seal-check copies alongside it) — never to cycle-1 paths, never over superseded evidence. Every entry point refuses to start when a target root exists and is non-empty; overwriting preserved evidence requires a new protocol version, not a flag.
...
| EG0 canonical commit, runtime, output roots, seal format | 105–118 | §9 (commit recorded at Task 8; format frozen here; provenance gate; tooling roots) |
...
## 13. Cycle-3 Task 7 checks, re-evaluation mode, and sample-share rule (to be executed post-approval)
...
- **Re-evaluation mode (single authorized use):** `uv run python experiments/sprint14_task7_proof.py --reevaluate` reads the exact preserved seed-796 roots (`data/generated/sprint14-task7-c3/full-796a`, `tiny-797`) after verifying their recorded manifest hashes (full-796a `5b230719210364d6c5e0be0257ee6b3665c4518929cd88afccb2ed5397e78691`; tiny-797 `10102db8d082e0a3600cd659ab8f4ea3b84aa76b01dd6c388f7f92307a111735`) and shard hashes via deterministic reload; it never regenerates, mutates, or overwrites any root, refuses to start if the evidence path exists and is non-empty, and completes the remaining fixtures plus seal round-trips on verified-identical working copies into the new evidence path `artifacts/sprint-14/server-task7-cycle-3-reeval/`.
