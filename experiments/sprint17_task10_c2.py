"""Sprint 17 Task 10 — train and evaluate C2 patchification arms on Development.

Runs exactly one registered C2 arm per invocation (``--arm C2-A`` or ``--arm
C2-B``) under the frozen B0 parity contract: same Fit/Calibration/Development
...
C2-A is the parity control: fixed-width (W=32) windows at 50% overlap
(stride 16) via the overlap-explicit ``Sprint17C2Patchifier``; its lattice is
bitwise identical to B0, no support weights are emitted, and every downstream
mechanism is literally unchanged.  C2-B uses two fixed-width offset grids
(offsets 0/16, stride 32) with unit-mass support weights consumed only by the
unchanged weighted-mean file pooling; S_pred aggregation, timestep
localization, masking, and metric code are unchanged.
...
C2_ARMS = ("C2-A", "C2-B")
ARM_FILE_PREFIX = {"C2-A": "c2a", "C2-B": "c2b"}
#: V1Config stride recorded per arm (primary grid stride; inert metadata in
#: the train/score path, kept truthful for the geometry-compatibility guard).
ARM_CONFIG_STRIDE = {"C2-A": 16, "C2-B": 32}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 10 (Development only)")
...
def arm_config_dict(arm_id: str, model_seed: int) -> dict:
    """Frozen per-arm configuration: B0 values plus the declared C2 change.

    Only ``arm_id``, ``stride`` (primary grid stride; inert metadata in the
    train/score path), ``c2_grids``, ``adapter_description``, and
    ``support_weights`` differ from :func:`b0_config_dict`.  Patch width,
    model capacity, masking, objective, optimizer, schedule, batch rule,
    precision, and budget are identical by construction.
    """
    from representation.sprint17_c2 import ARM_ADAPTER_DESCRIPTION, arm_grids
    if arm_id not in C2_ARMS:
        raise ValueError(f"Task 10 runs only {C2_ARMS}, got {arm_id!r}")
    cfg = b0_config_dict(model_seed)
    cfg["arm_id"] = arm_id
    cfg["stride"] = ARM_CONFIG_STRIDE[arm_id]
...
def count_flops_reference(model, arm_id: str, t: int = FLOP_REFERENCE_T) -> int:
...
def build_model(cfg_dict: dict, device):
    ...
    patchifier = arm_patchifier(cfg_dict["arm_id"])
    if (patchifier.cfg.patch_size != cfg.patch_size
            or patchifier.cfg.stride != cfg.stride):
        raise ValueError("arm patchifier geometry incompatible with arm config")
...
def main() -> int:
    ...
    ap.add_argument("--arm", required=True, choices=list(C2_ARMS),
                    help="registered C2 arm to execute (one per invocation)")
    ...
    arm_patchifier_obj = _arm_patchifier(arm_id)
    arm_fit_patches = sum(arm_patchifier_obj.patchify(s).N for s in fit_pool)
    if not args.smoke and arm_id == "C2-A" and arm_fit_patches != fit_patches:
        raise ValueError(
            f"C2-A parity breach: arm patches {arm_fit_patches} != "
    ...
    for model_seed in seeds:
        ...
        flops = count_flops_reference(model, arm_id)
        model.eval()
...
def assemble_evidence(outdir: Path, arm_id: str, seeds: tuple, seed_records: dict,
    ...
    from representation.sprint17_c2 import ARM_ADAPTER_DESCRIPTION
