"""Sprint 15 Task 9 cycle 2: candidate-2 public-entry-point contract proof.

Runs the canonical local public entry point in fresh subprocesses for the two
disposable proof roots (seeds 1116/1117, role H-PROOF-3/4), then verifies
...
``artifacts/sprint-15/proof-candidate-2/`` and prints a machine-readable
summary (the worker records it in ``artifacts/sprint-15/task-9-cycle-2.md``).
...
PROOF_DIR = Path("artifacts/sprint-15/proof-candidate-2")
ROSTER = (("H-PROOF-3", 1116), ("H-PROOF-4", 1117))
...
def run_cli(role: str, seed: int, outdir: Path) -> dict:
    ...
    cmd = [sys.executable, "-m", "synth.cli", "--chronological",
           "--profile", "sprint15-v2", "--seed", str(seed),
           "--role", role, "--protocol", "sprint15-benchmark-protocol-v2",
           "--output", str(outdir)]
    proc = subprocess.run(
...
def main() -> int:
    ...
    for role, seed in ROSTER:
        ...
        if manifest.get("protocol") != B.S15_PROTOCOL_V2:
            errors.append("protocol-tag")
        if block.get("protocol") != B.S15_PROTOCOL_V2:
        ...
        audit = B.audit_sprint15(manifest, profile=B.S15_PROFILE_V2, protocol=B.S15_PROTOCOL_V2)
    ...
    rep = REPEAT_DIR / "H-PROOF-3"
    repeat = run_cli("H-PROOF-3", 1116, rep)
