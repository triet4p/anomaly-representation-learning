"""Sprint 15 Task 9 cycle 3: candidate-3 public-entry-point contract proof.

Runs the canonical local public entry point in fresh subprocesses for the two
disposable proof roots (seeds 1216/1217, role H-PROOF-5/6), then verifies
...
``artifacts/sprint-15/proof-candidate-3/`` and prints a machine-readable
summary (the worker records it in ``artifacts/sprint-15/task-9-cycle-3.md``).
...
PROOF_DIR = Path("artifacts/sprint-15/proof-candidate-3")
ROSTER = (("H-PROOF-5", 1216), ("H-PROOF-6", 1217))
...
def run_cli(role: str, seed: int, outdir: Path) -> dict:
    ...
    cmd = [sys.executable, "-m", "synth.cli", "--chronological",
           "--profile", "sprint15-v3", "--seed", str(seed),
           "--role", role, "--protocol", "sprint15-benchmark-protocol-v3",
           "--output", str(outdir)]
    proc = subprocess.run(
...
def main() -> int:
    ...
    for role, seed in ROSTER:
        ...
        if manifest.get("protocol") != B.S15_PROTOCOL_V3:
            errors.append("protocol-tag")
        if block.get("protocol") != B.S15_PROTOCOL_V3:
        ...
        audit = B.audit_sprint15(manifest, profile=B.S15_PROFILE_V3, protocol=B.S15_PROTOCOL_V3)
    ...
    rep = REPEAT_DIR / "H-PROOF-5"
    repeat = run_cli("H-PROOF-5", 1216, rep)
