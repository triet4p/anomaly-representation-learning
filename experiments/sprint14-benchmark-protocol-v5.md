# Sprint 14 Benchmark Recovery Protocol v5 (FROZEN, AWAITING GATE B1 APPROVAL — CYCLE 3 FINAL)

**Status:** Internally frozen by the Sprint 14 worker as the cycle-3 Task 5 freeze (Main-directed A+C). NOT APPROVED — no implementation, materialization, audit, probe execution, or outcome generation under v5 may start before Evidence Gate B1 passes with zero actionable findings. This is the third and final allowed diagnostic cycle: any Cycle 3 Design miss closes `NOT_MEASURABLE` after evidence review, with no deep reviewer. Any further semantic change requires a new protocol version, Main approval, and a fresh evidence review before materialization.
...
**Protocol identifier:** `sprint14-benchmark-protocol-v5`. This file's own SHA256 digest is recorded after freezing in `artifacts/sprint-14/task-5-cycle-3.md` and `artifacts/sprint-14/cycle-ledger.json` (self-embedding a digest would be circular).

**Diagnostic cycle:** cycle 3 of at most 3 (Cycles 1–2 consumed with `RETIRED-PROVENANCE-FAIL` and `DESIGN-FAIL` respectively; no cycles remain after this one).

## 0. Amendment record (v4.1 → v5; A+C availability/exposure correction, no rate/floor relaxation)

- `experiments/sprint14-benchmark-protocol-v1.md` (`8279e2b5…6c8b6697a1b`), `-v2.md` (`5b6eb212…7a7c67404`), `-v3.md` (`45b5ef69…fb85d1`), `-v4.md` (`88f2b426…bb3176`), `-v4.1.md` (`66572d72…03a5ebb4`) preserved byte-identical. Cycle 1 roots/audits retired (`e018`); Cycle 2 roots preserved as diagnostic-only, never reusable for promotion; quarantined Cycle 1 numbers are not cited anywhere in this file.
- Exact deltas, nothing else:
  1. **Preventive maintenance cadence (§1):** 30 d → 60 d per robot; preventive duration unchanged (1 d).
  2. **Corrective downtime (§1):** 2 d → 1 d per failure episode (episode count behavior unchanged; only the recommissioning closure shortens).
  3. **Uniform exposure increase (§1):** exactly 240 calendar days for every Design/Fit/Calibration/Confirmation/Sealed history (span 180 → 240 d; development cutoff 120 → 160 d proportionally); unit throughput preserved (arrival interval/jitter unchanged at 11200 s; unit count 1152 → 1536 so per-robot op cadence φ ≈ 1.6 ops/robot-day is unchanged).
  4. **Fresh full roster and root identities (§2):** Design H-DESIGN-9…12 (780–783), Fit H-FIT-7…9 (784–786), Calibration H-CAL-3 (787), Confirmation H-CONF-6…9 (788–791), Sealed H-SEAL-9…12 (792–795); Task 7 proof seeds 796 (full) / 797 (tiny) with roots `data/generated/sprint14-task7-c3/` and reports `server-task7-cycle-3(-rerun)/`. Proven disjoint in Task 5 §6, not assumed.
  5. **Stale audit-tag correction predeclared (§§9, 12):** the Task 9 audit asserts the v5 identifier (the v4.1 literal is superseded); Task 6 retargets audit roster/roots/output (`design-audit-cycle-3.json`), proof constants/paths, and extends the seal allowlist with the v5 tag — tooling only, executed after B1 and reviewed with Task 7.
  6. **Canonical implementation commit pending Task 6 (§9):** no code below is implemented or committed by this freeze. Gate B1 approves this frozen amendment specification only — not EG0 materialization, which additionally requires the committed-bytes + Task 7 proof gates before Task 8.
- This is an availability/exposure correction (fewer maintenance shadows; more evaluable robot-time), not a rate change (hazard/wear/threshold laws untouched) and not a floor relaxation (every floor, margin, and threshold transcribed verbatim).

## 1. Cycle-3 DGP (frozen settings)
...
- Frozen route table (Option A geometry; identical to v3/v4/v4.1):
...
Every robot serves exactly one stage. Unit allocation across routes, per-stage op volumes per unit time, and total throughput rate are unchanged; total exposure scales with the longer calendar.

- Health/hazard (`HealthConfig`, seed = history seed): aging `1e-7`, pre-episode wear `3.0e-5`, noise `1e-3`, onset `0.3`, `alpha = 5.0`, severity scale `2.0`, `upcoming_p = 0.55`; corrective maintenance **1 d**, preventive every **60 d + 1 d**; cohorts `P(share 0.45, base 5.0e-10, wear 2.0e-4, H* 2.0)`, `W(share 0.30, base 3.0e-9, wear 5.0e-5, H* 1.35, κ 0.3)`, `A(share 0.25, abrupt_rate 1.1e-5 s⁻¹, subtypes A1/A2)` — rates, shares, thresholds, and gains numerically identical to v4.1; only maintenance timing and calendar span change.
...
Freshness statement (proven in Task 5 §6, not assumed): the 16 roster seeds plus Task 7 proof seeds 796/797 are pairwise distinct; their intersection with the v1–v3 band (700–733), cycle-1/2 seeds ledgered to date, disposable proof seeds (740, 741), all 45 historical seeds, and test-generation seeds (910–914, 931–933) is strictly empty. Root identities continue past every prior H- family in both sprints. Cycle-3 Design materialization uses fresh roots under `data/generated/sprint14-v5/`; retired/quarantined roots are never touched.
...
- One bounded DGP amendment per diagnostic cycle (this v5 is the cycle-3 amendment), driven only by design-role structural diagnostics and the frozen probe. Learned representation, anomaly, warning, and calibrated-risk outputs cannot select generator settings, histories, thresholds, or gates.
...
- Generation uses the `--profile sprint14-v3` code path (DGP implementation extended at Task 6 with the §1 maintenance/span settings; Task 6 owns the minimal implementation after Gate B1 approval — no implementation under v5 before Gate B1) with an explicit protocol tag and fresh roots: `uv run python -m synth.cli --chronological --profile sprint14-v3 --seed <S> --role <R> --protocol sprint14-benchmark-protocol-v5 --output data/generated/sprint14-v5/<ROLE>/`.
- Explicit local roots: `data/generated/sprint14-v5/<ROLE>/` under the repository, gitignored for bulk bytes. Bounded manifests committed per history (manifest SHA256, config hash, role, protocol tag, seeds, counts, timing, episode-subtype tallies); bulk bytes never committed. Seals via `write_seal`/`verify_seal` with the v5 protocol tag (allowlist extended at Task 6 only if the committed code requires it — identified before B1, never after outcomes); manifest digests round-trip verified and digest-recorded in the cycle ledger. First-attempt status recorded; no retry/replacement.
- Canonical commit, runtime versions, output roots, and seal format are recorded in the cycle ledger at materialization time (Task 8), not invented here. The exact canonical implementation commit is pending Task 6 and must be independently evidence-reviewed with Task 7 before Task 8; Gate B1 approves this frozen amendment specification only, not EG0 materialization.
...
- **Tooling roots (collision-free):** Task 7 proof entry point writes cycle-3 runs to `artifacts/sprint-14/server-task7-cycle-3(-rerun)/task7.json` with disposable data roots under `data/generated/sprint14-task7-c3(-rerun)/` (seeds 796 full / 797 tiny) — never to cycle-1/2 proof paths. The entry point refuses to start when a target root exists and is non-empty; overwriting preserved evidence requires a new protocol version, not a flag. The Task 9 audit entry point is retargeted to roster H-DESIGN-9…12, seeds 780–783, roots `data/generated/sprint14-v5/`, output `artifacts/sprint-14/design-audit-cycle-3.json`, and asserts the v5 identifier (the v4.1 literal is superseded) — tooling only, executed after B1 and reviewed with Task 7.
...
|---|---|---|
| EG0 protocol version + content digest | 105–118 | Header (identifier, digest in task-5-cycle-3.md/ledger); §8 (post-generation edits fail EG0); §9 (provenance gate) |
...
| EG0 canonical commit, runtime, output roots, seal format | 105–118 | §9 (commit pending Task 6; runtime/roots recorded at Task 8; format frozen here; provenance gate; tooling roots) |
...
## 13. Cycle-3 Task 7 subtype causal/physics checks (to be executed post-B1)
