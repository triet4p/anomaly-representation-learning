"""Sprint 15 Task 9 cycle 6: candidate-6 public-entry-point contract proof.

Runs the canonical local public entry point in fresh subprocesses for the two
disposable proof roots (seeds 1516/1517, role H-PROOF-11/12), then verifies
...
nothing outside ``artifacts/sprint-15/proof-candidate-6/`` and prints a
machine-readable summary (the worker records it in
``artifacts/sprint-15/task-9-cycle-6.md``).
...
PROOF_DIR = Path("artifacts/sprint-15/proof-candidate-6")
ROSTER = (("H-PROOF-11", 1516), ("H-PROOF-12", 1517))
...
def run_cli(role: str, seed: int, outdir: Path) -> dict:
    ...
    cmd = [sys.executable, "-m", "synth.cli", "--chronological",
           "--profile", "sprint15-v6", "--seed", str(seed),
           "--role", role, "--protocol", "sprint15-benchmark-protocol-v6",
           "--output", str(outdir)]
    proc = subprocess.run(
...
def main() -> int:
    ...
    for role, seed in ROSTER:
        ...
        if manifest.get("protocol") != B.S15_PROTOCOL_V6:
            errors.append("protocol-tag")
        if block.get("protocol") != B.S15_PROTOCOL_V6:
            errors.append("block-protocol")
        if block.get("profile") != B.S15_PROFILE_V6:
        ...
        audit = B.audit_sprint15(manifest, profile=B.S15_PROFILE_V6, protocol=B.S15_PROTOCOL_V6)
