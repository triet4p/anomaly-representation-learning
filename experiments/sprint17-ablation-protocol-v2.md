# Sprint 17 Controlled Suspect-Component Ablation Protocol v2

**Status: FROZEN PROSPECTIVELY — no Sprint 17 outcome exists in this protocol.**
**Protocol ID:** `sprint17-ablation-v2`
...
This v2 amendment is limited to the B0 schema representation defect identified in the Task 1 evidence review: B0 is a baseline with no replaced component, so its required `component_set` is `[]`. v1 is preserved as historical/superseded. No data, model, arm, metric, threshold, promotion rule, stopping rule, or Sealed boundary changes. Any semantic change after this freeze requires `sprint17-ablation-v3` (or later), a fresh evidence review, and an explicit prospective decision before any affected outcome is inspected. No result may select a threshold, seed, arm, combination, checkpoint, or metric.
...
6. Confirmation opens only after the complete Task 23 lock; any lock mismatch bars Confirmation permanently for v2.
...
```json
{
  ...
  "properties": {
    "schema_id": {"const": "sprint17-ablation-result-v2"},
    "protocol_id": {"const": "sprint17-ablation-v2"},
