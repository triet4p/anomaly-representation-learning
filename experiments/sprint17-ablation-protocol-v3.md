# Sprint 17 Controlled Suspect-Component Ablation Protocol v3

**Status: FROZEN PROSPECTIVELY — no Sprint 17 outcome exists in this protocol.**
**Protocol ID:** `sprint17-ablation-v3`
**Scope:** Tasks 2A–2B only. This document is the complete prospective pre-outcome contract for the v3 restart; it does not authorize preflight, root creation, generation, implementation, training, scoring, or Confirmation opening by itself.

This v3 amendment supersedes no history: v1 and v2 remain byte-preserved historical protocols. It corrects no scientific contract. The only authorized semantic change is abandoning v2's complete failed roster and binding exactly one all-new v3 roster; all DGP, quota, gate, architecture, metric, model, budget, promotion, stopping, and Sealed rules below remain unchanged.


## 0. Protocol-v3 amendment and permanent v2 failure
...
6. Confirmation opens only after the complete Task 23 lock; any lock mismatch bars Confirmation permanently for v3.
...
```json
{
  ...
  "properties": {
    "schema_id": {"const": "sprint17-ablation-result-v3"},
    "protocol_id": {"const": "sprint17-ablation-v3"},
