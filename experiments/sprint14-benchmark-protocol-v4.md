# Sprint 14 Benchmark Recovery Protocol v4 (FROZEN, AWAITING GATE B1 APPROVAL — CYCLE 2)

**Status:** Internally frozen by the Sprint 14 worker as the cycle-2 Task 5 freeze (Main-directed R1). NOT APPROVED — no implementation, materialization, audit, probe execution, or outcome generation under v4 may start before a fresh Evidence Gate B1 passes with zero actionable findings. Any further semantic change requires a new protocol version (`v5`, …), Main approval, and a fresh evidence review before materialization.
...
**Protocol identifier:** `sprint14-benchmark-protocol-v4`. This file's own SHA256 digest is recorded after freezing in `artifacts/sprint-14/task-5-cycle-2.md` and `artifacts/sprint-14/cycle-ledger.json` (self-embedding a digest would be circular).
...
## 0. Amendment record (v3 → v4; v1–v3 preserved, no DGP change)

- `experiments/sprint14-benchmark-protocol-v1.md` (`8279e2b5…6c8b6697a1b`), `-v2.md` (`5b6eb212…7a7c67404`), `-v3.md` (`45b5ef69…fb85d1`) preserved byte-identical. Cycle 1 roots/audits retired (`e018`); quarantined Cycle 1 numbers are not cited anywhere in this file.
- Exact deltas, nothing else:
  1. **Roster and root identities (§2):** entirely fresh full roster — Design H-DESIGN-5…8 (seeds 742–745), Fit H-FIT-4…6 (750–752), Calibration H-CAL-2 (753), Confirmation H-CONF-2…5 (760–763), Sealed H-SEAL-5…8 (770–773). Every seed and root identity is unused and was never declared under v1–v3 (which used seeds 700–733 only), never materialized as a proof root (740, 741), and is outside all 45 historical seeds. Numeric continuation past every prior H- family (Sprint 13 H-FIT-1…3/H-CAL-1/H-CONF-1/H-SEAL-1…4; Sprint 14 cycle-1 H-DESIGN-1…4) avoids every retired identity in both sprints.
  2. **Provenance gate (§9):** canonical-commit rule — materialization permitted only with all runtime-affecting bytes committed and the working tree verified clean for those paths before the first run; artifact-only bookkeeping dirt allowed only if explicitly enumerated and hashed; any runtime diff aborts before generation. Canonical commit for cycle 2 is recorded in `task-5-cycle-2.md` and the ledger, not in this file (a file cannot contain its own commit's hash).
  3. **Code-profile naming (§9):** the DGP implementation is byte-identical to v3, so generation uses the existing `--profile sprint14-v3` code path with `--protocol sprint14-benchmark-protocol-v4` and output roots `data/generated/sprint14-v4/<ROLE>/`. Profile name denotes code bytes; protocol tag denotes freeze identity. No new behavior is implemented for v4.
- The v3 DGP (Option A routes, physical P2/W2 fork, all rates/physics/semantics/floors/probe) is otherwise carried forward byte-for-meaning identical. Predictions from Task 5 cycle 1 stand unmodified as the still-untested hypothesis.

## 1. Cycle-2 DGP (identical to v3; frozen settings restated)
...
- Frozen route table (Option A; identical to v3):
...
Freshness statement (proven, not asserted): the 16 seeds are pairwise distinct; their intersection with the v1–v3 declared band (700–733), the disposable proof seeds (740, 741), the retired cycle-1 seeds (700–703, subset of the band), and all 45 historical seeds (100–103, 200–203, 300–308, 400–403, 500–508, 600–603, 920–927, 929, 930, 13) is strictly empty — verified by the disjointness check recorded in `task-5-cycle-2.md` §6. Root identities continue past every prior H- family in both sprints, so no directory, manifest, seal, ledger, or audit record can be confused with a retired one. Cycle-2 Design materialization uses fresh roots under `data/generated/sprint14-v4/`; the retired `sprint14-v3/H-DESIGN-*` roots are never touched.
...
- One bounded DGP amendment per diagnostic cycle (this v4 carries the cycle-1 amendment forward unchanged into cycle 2), driven only by design-role structural diagnostics and the frozen probe. Learned representation, anomaly, warning, and calibrated-risk outputs cannot select generator settings, histories, thresholds, or gates.
...
- If fresh v4 Design histories miss any promotion target, Gate B2 evidence review occurs first; Main then chooses the next methodology from the new evidence. No Cycle 3 methodology is pre-authorized by this protocol.

## 9. Canonical runtime, roots, seals, and provenance gate (frozen; local-only)
...
- Generation uses the existing `--profile sprint14-v3` code path (DGP bytes identical to v3) with an explicit protocol tag and fresh roots: `uv run python -m synth.cli --chronological --profile sprint14-v3 --seed <S> --role <R> --protocol sprint14-benchmark-protocol-v4 --output data/generated/sprint14-v4/<ROLE>/` (Task 6 owns no new implementation for v4 — there is no behavior change to implement; Task 7 re-runs at the committed bytes before Task 8).
- Explicit local roots: `data/generated/sprint14-v4/<ROLE>/` under the repository, gitignored for bulk bytes. Bounded manifests committed per history (manifest SHA256, config hash, role, protocol tag, seeds, counts, timing, episode-subtype tallies); bulk bytes never committed. Seals via `write_seal`/`verify_seal` with the v4 protocol tag (allowlist extended at Task 6 of cycle 2 only if the committed code requires it — identified before B1, never after outcomes); manifest digests round-trip verified and digest-recorded in the cycle ledger. First-attempt status recorded; no retry/replacement.
...
- **Provenance gate (binding pre-materialization check):** no Task 8 run may start unless (a) every runtime-affecting byte (the committed source/config/test/proof/audit set recorded in `task-5-cycle-2.md` §7 and ledger `e021`) matches the canonical commit byte-for-byte (`git status` clean for those paths and blob hashes equal); (b) any other working-tree dirt is confined to the explicitly enumerated, hashed artifact-only bookkeeping set (same sources); (c) any runtime diff aborts generation before the first history. A base commit plus uncommitted runtime changes is not a canonical commit.
...
|---|---|---|
| EG0 protocol version + content digest | 105–118 | Header (identifier, digest in task-5-cycle-2.md/ledger); §8 (post-generation edits fail EG0); §9 (provenance gate) |
...
| EG0 canonical commit, runtime, output roots, seal format | 105–118 | §9 (commit/runtime/roots recorded at Task 8; format frozen here; provenance gate) |
...
## 13. Cycle-2 Task 7 subtype causal/physics checks (to be executed post-B1)
