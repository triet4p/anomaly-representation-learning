"""Sprint 17 Task 12 — train and evaluate C5 objective/masking arms on Development.

Runs exactly one registered C5 arm per invocation (``--arm C5-A`` or ``--arm
C5-B``) under the frozen B0 parity contract: same Fit/Calibration/Development
roles, model seeds ``[171701, 171702, 171703]``, exactly 300 optimizer steps
per trainable seed, step-300 checkpoint, q95 Calibration thresholds, and the
verbatim B0 metric code (paired arm-minus-B0 deltas on identical support).

C5-A is structured-predict: the frozen 0.40 mask quota as seeded contiguous
channel-time blocks plus shared-head multi-horizon (0/+1/+2) EMA latent
prediction. C5-B is redundancy-control: EMA masked prediction retained, the
file-level InfoNCE replaced by VICReg-style variance/covariance under the
identical frozen warmup (weight exactly 0.0 through step 300, so C5-B trains
the prediction-only trajectory — a parity control). Architecture, inputs,
total mask ratio, and model-selection basis stay frozen; neither arm adds
parameters.
...
rescue, no B0 retraining (B0 is reused from hash-verified Task 7 evidence).
...
so file scores are batching- and order-independent. Training masks use
...
C5_ARMS = ("C5-A", "C5-B")
ARM_FILE_PREFIX = {"C5-A": "c5a", "C5-B": "c5b"}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 12 (Development only)")
...
def arm_config_dict(arm_id: str, model_seed: int) -> dict:
    """Frozen per-arm configuration: B0 values plus the declared C5 change.

    Only ``arm_id``, ``masking_policy``, ``criterion``,
    and ``adapter_description`` differ from :func:`b0_config_dict`.
    Architecture, inputs, total mask ratio, model-selection basis, optimizer,
    schedule, batch rule, precision, and budget are identical by
    construction.
    """
    from representation.sprint17_c5 import ARM_ADAPTER_DESCRIPTION, C5A_HORIZONS
    if arm_id not in C5_ARMS:
        raise ValueError(f"Task 12 runs only {C5_ARMS}, got {arm_id!r}")
...
def count_flops_reference(model, t: int = FLOP_REFERENCE_T) -> int:
    ...
    try:
        ...
        # C5 keeps B0 patchification: the reference batch uses the B0 lattice,
        # so the arm count is directly comparable to the B0 reference.
...
def build_model(cfg_dict: dict, device):
    ...
    # C5 keeps the B0 graph exactly (patchifier and encoder); only masking
    # inputs and the training criterion change. Parameter identity is
    # asserted: C5 adds no learnable parameters.
    ...
    if count_parameters(model) != B0_PARAMS:
        raise ValueError("C5 parameter identity breach: C5 adds no parameters")
    model.to(device)
    return model, cfg, patchifier

def train_seed(arm_id: str, model_seed: int, fit_pool: list, device, log: list,
    ...
    criterion = arm_criterion(
        arm_id, temperature=cfg_dict["contrastive_temperature"],
        prediction_weight=cfg_dict["prediction_weight"],
    ...
    trainer = RepresentationTrainer(model, criterion=criterion, optimizer=optimizer, scheduler=scheduler,
...
def main() -> int:
    ...
    ap.add_argument("--arm", required=True, choices=list(C5_ARMS),
                    help="registered C5 arm to execute (one per invocation)")
    ...
    # C5 keeps B0 patchification exactly: the arm patch count must equal the
    ...
    if not args.smoke and arm_fit_patches != fit_patches:
        raise ValueError(
            f"C5 lattice breach: arm patches {arm_fit_patches} != "
...
def assemble_evidence(outdir: Path, arm_id: str, seeds: tuple, seed_records: dict,
    ...
    from representation.sprint17_c5 import ARM_ADAPTER_DESCRIPTION
