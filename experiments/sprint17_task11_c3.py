"""Sprint 17 Task 11 — train and evaluate C3 local-encoder arms on Development.

Runs exactly one registered C3 arm per invocation (``--arm C3-A`` or ``--arm
C3-B``) under the frozen B0 parity contract: same Fit/Calibration/Development
...
C3-A is a parameter-matched residual depthwise-separable dilated 1D
convolutional patch encoder (34,688 params vs B0 35,712); C3-B is a
parameter-matched lightweight within-patch Transformer with explicit position
and padding masks (35,724 vs 35,712). Only the patch-to-local-latent function
changes: patchification, context encoder, objective, pooling, geometry,
scorers, aggregation, and metric code are unchanged.
...
C3_ARMS = ("C3-A", "C3-B")
ARM_FILE_PREFIX = {"C3-A": "c3a", "C3-B": "c3b"}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 11 (Development only)")
...
def arm_config_dict(arm_id: str, model_seed: int) -> dict:
    """Frozen per-arm configuration: B0 values plus the declared C3 change.

    Only ``arm_id``, ``local_encoder``, ``local_encoder_params``,
    and ``adapter_description`` differ from :func:`b0_config_dict`.
    Patchification, model capacity elsewhere, masking, objective, optimizer,
    schedule, batch rule, precision, and budget are identical by
    construction.
    """
    from representation.sprint17_c3 import (
        ARM_ADAPTER_DESCRIPTION, ARM_LOCAL_ENCODER_PARAMS, arm_local_encoder)
    if arm_id not in C3_ARMS:
        raise ValueError(f"Task 11 runs only {C3_ARMS}, got {arm_id!r}")
...
def count_flops_reference(model, t: int = FLOP_REFERENCE_T) -> int:
    ...
    try:
        ...
        # C3 keeps B0 patchification: the reference batch uses the B0 lattice,
        # so the count isolates the local-encoder FLOP delta.
...
def build_model(cfg_dict: dict, device):
    ...
    # C3 keeps B0 patchification exactly; only the local encoder is swapped.
    patchifier = Patchifier(PatchConfig(patch_size=cfg.patch_size,
                                        stride=cfg.stride,
                                        pad_end=cfg_dict["pad_end"]))
    patch_encoder = arm_local_encoder(
        cfg_dict["arm_id"], cfg.n_channels, cfg.d_model, cfg.dropout)
    if count_local_encoder_params(patch_encoder) != ARM_LOCAL_ENCODER_PARAMS[
            cfg_dict["arm_id"]]:
        raise ValueError("arm local-encoder parameter identity mismatch")
...
def main() -> int:
    ...
    ap.add_argument("--arm", required=True, choices=list(C3_ARMS),
                    help="registered C3 arm to execute (one per invocation)")
    ...
    # C3 keeps B0 patchification exactly: the arm patch count must equal the
    ...
    if not args.smoke and arm_fit_patches != fit_patches:
        raise ValueError(
            f"C3 lattice breach: arm patches {arm_fit_patches} != "
...
def assemble_evidence(outdir: Path, arm_id: str, seeds: tuple, seed_records: dict,
    ...
    from representation.sprint17_c3 import ARM_ADAPTER_DESCRIPTION
