"""Sprint 17 Task 14 — evaluate C7 geometry/bank arms on Development (no training).

Runs exactly one registered C7 arm per invocation (``--arm C7-A`` or ``--arm
C7-B``) under the frozen B0 parity contract: same Fit/Calibration/Development
...
C7-A is per-condition robust location plus Ledoit-Wolf-style covariance
shrinkage with the frozen (robot,program)->robot->program->global hierarchy
and fallback, scored as squared-Mahalanobis energy; C7-B is condition-aware
kNN (k=5) mean distance over a robust Fit-only scale. Only the population
reference/scoring function changes: representations, targets, S_pred,
projector, calibration rule, support, and metric code are unchanged, and
S_pred is reused bitwise from the hash-verified B0 caches.
...
rescue, no B0 retraining (B0 states, caches, and S_pred reused from hash-verified
...
Development embeddings come from the reloaded frozen B0 forward; Development
embeddings and S_pred are additionally asserted against the hash-verified B0
caches (fidelity proof) and S_pred is reused bitwise from those caches.
"""
...
C7_ARMS = ("C7-A", "C7-B")
ARM_FILE_PREFIX = {"C7-A": "c7a", "C7-B": "c7b"}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 14 (Development only)")
...
def arm_config_dict(arm_id: str, model_seed: int) -> dict:
    """Frozen per-arm configuration: B0 values plus the declared C7 change.

    Only ``arm_id``, ``reference``, and ``adapter_description`` differ
    from :func:`b0_config_dict`. Representations, targets, pooling,
    objective, optimizer, schedule, batch rule, precision, and budget are
    identical by construction; this driver executes zero optimizer steps
    (representations inherit the frozen B0 step-300 states).
    """
    from representation.sprint17_c7 import ARM_ADAPTER_DESCRIPTION, arm_reference
    if arm_id not in C7_ARMS:
        raise ValueError(f"Task 14 runs only {C7_ARMS}, got {arm_id!r}")
...
def count_flops_reference(model, t: int = FLOP_REFERENCE_T) -> int:
    ...
    try:
        ...
        # C7 keeps B0 patchification and encoding: the reference batch uses the
...
def build_model(cfg_dict: dict, device):
    ...
    # C7 keeps the B0 graph exactly (frozen representations); only the
    # population reference/scorer is replaced downstream of the forward.
    ...
    if count_parameters(model) != B0_PARAMS:
        raise ValueError("C7 model graph is not the frozen B0 graph")
    if count_flops_reference(model) != B0_FLOPS_REFERENCE:
        raise ValueError("C7 scoring FLOPs differ from the B0 reference")
    model.to(device)
...
def main() -> int:
    ...
    ap.add_argument("--arm", required=True, choices=list(C7_ARMS),
                    help="registered C7 arm to execute (one per invocation)")
    ...
    # C7 keeps B0 patchification exactly: the arm patch count must equal the
    ...
    if not args.smoke and arm_fit_patches != fit_patches:
        raise ValueError(
            f"C7 lattice breach: arm patches {arm_fit_patches} != "
    ...
    # ---- Per-seed frozen reload / embed / C7 fit / calibrate / score ----
    ...
    for model_seed in seeds:
        ...
        # 2. Hash-verified B0 Development caches: bitwise S_pred + embeddings.
        b0_dev = {}
        for role in dev_roles:
            b0_dev[role] = load_b0_cache(b0_cache_dir, model_seed, role)
        # 3. Frozen forward over Fit / Calibration / Development (B0 bank kept
        # for the S_pop fidelity comparison only; it never enters C7 scores).
        fit_pred, _, fit_emb, _ = score_items(model, b0_bank, patchifier, cfg,
                                              fit_items, device)
        if not torch.isfinite(fit_emb).all():
            raise ValueError(f"non-finite Fit embeddings: seed {model_seed}")
        cal_pred, _, cal_emb, _ = score_items(model, b0_bank, patchifier, cfg,
                                              cal_items, device)
        # 4. Fidelity proof: reloaded forward must reproduce the hash-verified
        # B0 Development caches (fail closed above FIDELITY_ATOL).
        fidelity: dict[str, float] = {}
        for role in dev_roles:
            items = [(r["file_id"], dev_by_id[role][r["file_id"]])
                     for r in dev_rows[role] if r["file_id"] in dev_by_id[role]]
            f_pred, f_pop, f_emb, _ = score_items(model, b0_bank, patchifier,
                                                  cfg, items, device)
            z = b0_dev[role]
        ...
        # 5. Fit the C7 reference on Fit forward embeddings (Fit-only healthy
        # rows; abnormal labels rejected inside fit).
        fit_robots, fit_programs = condition_lists(fit_items)
        fit_labels = [sample.file_label for _, sample in fit_items]
        reference = arm_reference(arm_id).fit(fit_emb, fit_robots, fit_programs,
                                              labels=fit_labels)
        # 6. C7 S_pop on cached (bitwise-B0) Development embeddings and forward
        # Calibration embeddings.
        cal_robots, cal_programs = condition_lists(cal_items)
        cal_pop_c7, _ = reference.score(cal_emb, cal_robots, cal_programs)
        cal_pop_c7 = np.asarray(cal_pop_c7, dtype=np.float64)
        # 7. Thresholds: S_pred reused bitwise from B0 (proof: recomputed q95
        # on forward Calibration S_pred matches outside smoke); S_pop fresh q95.
        b0_thr_pred = float(b0_pred["seed_records"][str(model_seed)]["thr_pred"])
        if not args.smoke:
            recal = P.select_threshold(cal_pred)
            if abs(recal - b0_thr_pred) > FIDELITY_ATOL:
                raise ValueError(f"S_pred threshold proof failed: seed {model_seed}")
        thr_pred = b0_thr_pred
        thr_pop = P.select_threshold(cal_pop_c7)
        if not (np.isfinite(thr_pred) and np.isfinite(thr_pop)):
            raise ValueError(f"non-finite calibration threshold: seed {model_seed}")
        # Fit-healthy scores for the stability denominator (forward S_pred,
        # C7 S_pop on the same Fit embeddings).
        fit_pop_c7, _ = reference.score(fit_emb, fit_robots, fit_programs)
        fit_pop_c7 = np.asarray(fit_pop_c7, dtype=np.float64)
        dev_out: dict[str, dict] = {}
        for role in dev_roles:
            z = b0_dev[role]
            pos = {str(fid): i for i, fid in enumerate(list(z["file_ids"]))}
            ids = [r["file_id"] for r in dev_rows[role]
            ...
            ttime = [np.asarray(tpad[i, :tlen[i]], dtype=np.float64)
                     for i in range(len(ids))]
            d_robots = [int(dev_by_id[role][fid].robot_idx) for fid in ids]
            d_programs = [int(dev_by_id[role][fid].program_idx) for fid in ids]
            ...
            dev_out[role] = {"ids": ids, "S_pred": pred,
        ...
        # Persist C7 caches (same contract/keys as B0; S_pred reused bitwise).
        ...
        for role in dev_roles:
            ...
            z = b0_dev[role]
            pos = {str(fid): i for i, fid in enumerate(list(z["file_ids"]))}
            idx = [pos[fid] for fid in d["ids"]]
            tpad = np.asarray(z["timestep_scores"])[idx].astype(np.float32)
            tlen = np.asarray(z["timestep_lengths"])[idx].astype(np.int64)
        ...
        seed_records[model_seed] = {
            ...
            "reference_kind": type(reference).__name__,
            ...
            "dev_out": dev_out, "fit_pred": fit_pred, "fit_pop": fit_pop_c7,
            "cfg_dict": cfg_dict,
        }
...
def assemble_evidence(outdir: Path, arm_id: str, seeds: tuple, seed_records: dict,
    ...
    from representation.sprint17_c7 import ARM_ADAPTER_DESCRIPTION
