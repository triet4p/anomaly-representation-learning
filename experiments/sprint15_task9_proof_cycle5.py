"""Sprint 15 Task 9 cycle 5: candidate-5 public-entry-point contract proof.

Runs the canonical local public entry point in fresh subprocesses for the two
disposable proof roots (seeds 1416/1417, role H-PROOF-9/10), then verifies
...
nothing outside ``artifacts/sprint-15/proof-candidate-5/`` and prints a
machine-readable summary (the worker records it in
``artifacts/sprint-15/task-9-cycle-5.md``).
...
PROOF_DIR = Path("artifacts/sprint-15/proof-candidate-5")
ROSTER = (("H-PROOF-9", 1416), ("H-PROOF-10", 1417))
...
def run_cli(role: str, seed: int, outdir: Path) -> dict:
    ...
    cmd = [sys.executable, "-m", "synth.cli", "--chronological",
           "--profile", "sprint15-v5", "--seed", str(seed),
           "--role", role, "--protocol", "sprint15-benchmark-protocol-v5",
           "--output", str(outdir)]
    proc = subprocess.run(
...
def main() -> int:
    ...
    for role, seed in ROSTER:
        ...
        if manifest.get("protocol") != B.S15_PROTOCOL_V5:
            errors.append("protocol-tag")
        if block.get("protocol") != B.S15_PROTOCOL_V5:
            errors.append("block-protocol")
        if block.get("profile") != B.S15_PROFILE_V5:
        ...
        audit = B.audit_sprint15(manifest, profile=B.S15_PROFILE_V5, protocol=B.S15_PROTOCOL_V5)
    ...
    rep = REPEAT_DIR / "H-PROOF-9"
    repeat = run_cli("H-PROOF-9", 1416, rep)
