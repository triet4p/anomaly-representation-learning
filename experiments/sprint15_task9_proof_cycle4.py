"""Sprint 15 Task 9 cycle 4: candidate-4 public-entry-point contract proof.

Runs the canonical local public entry point in fresh subprocesses for the two
disposable proof roots (seeds 1316/1317, role H-PROOF-7/8), then verifies
...
nothing outside ``artifacts/sprint-15/proof-candidate-4/`` and prints a
machine-readable summary (the worker records it in
``artifacts/sprint-15/task-9-cycle-4.md``).
...
PROOF_DIR = Path("artifacts/sprint-15/proof-candidate-4")
ROSTER = (("H-PROOF-7", 1316), ("H-PROOF-8", 1317))
...
def run_cli(role: str, seed: int, outdir: Path) -> dict:
    ...
    cmd = [sys.executable, "-m", "synth.cli", "--chronological",
           "--profile", "sprint15-v4", "--seed", str(seed),
           "--role", role, "--protocol", "sprint15-benchmark-protocol-v4",
           "--output", str(outdir)]
    proc = subprocess.run(
...
def main() -> int:
    ...
    for role, seed in ROSTER:
        ...
        if manifest.get("protocol") != B.S15_PROTOCOL_V4:
            errors.append("protocol-tag")
        if block.get("protocol") != B.S15_PROTOCOL_V4:
            errors.append("block-protocol")
        if block.get("profile") != B.S15_PROFILE_V4:
        ...
        audit = B.audit_sprint15(manifest, profile=B.S15_PROFILE_V4, protocol=B.S15_PROTOCOL_V4)
    ...
    rep = REPEAT_DIR / "H-PROOF-7"
    repeat = run_cli("H-PROOF-7", 1316, rep)
