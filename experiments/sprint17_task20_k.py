"""Sprint 17 Tasks 20–21 — train and evaluate K1–K9 stack/pair combinations.

Runs exactly one frozen K combination per invocation (``--k K1`` … ``--k K9``)
under the frozen B0 parity contract: same Fit/Calibration/Development roles,
model seeds ``[171701, 171702, 171703]``, exactly 300 optimizer steps per
trainable seed, step-300 checkpoint, q95 Calibration thresholds, and the
verbatim B0 metric code (paired K-minus-B0 deltas on identical support).

Each K arm copies the exact Task 17 selected single-arm implementations with
zero combination-specific tuning (Task 19 freeze at
``experiments/sprint17-task19-combinations.json``, validated by hash before
any execution below):

- ``K1`` (C2-A+C3-A): C2-A overlap-explicit patchifier (bitwise B0 lattice)
  with the C3-A residual dilated depthwise-separable patch encoder; trained.
- ``K2`` (C3-A+C5-A): C3-A encoder with C5-A channel-time-block masking and
  multi-horizon EMA criterion; trained.
- ``K3`` (C5-A+C6-A): C5-A masking/criterion with C6-A mean-plus-std file
  pooling; trained.
- ``K4`` (C6-A+C7-A): C6-A pooling trained; S_pop rescored with the C7-A
  robust-shrinkage reference fit on the K4 Fit embeddings.
- ``K5`` (C7-A+C8-A): train-free on hash-verified B0 states — C8-A Huber
  S_pred plus C7-A S_pop.
- ``K6`` (C8-A+C9-B): train-free on hash-verified B0 states — C9-B
  contiguous-window aggregation over C8-A Huber patch energies; S_pop
  bitwise B0.
- ``K7`` (C2-A+C3-A+C5-A+C6-A): complete upstream stack trained (C2-A
  patchifier, C3-A encoder, C5-A masking/criterion, C6-A pooling).
- ``K8`` (C7-A+C8-A+C9-B): train-free on hash-verified B0 states —
  C9-B windows over C8-A energies (S_pred) plus C7-A S_pop.
- ``K9`` (all seven): upstream stack trained as K7; S_pred rescored with
  C8-A+C9-B and S_pop with C7-A on the K9 forward latents.
...
no threshold tuning, no early stopping or best-state
selection (final step-300 state is the checkpoint), no extra gradient runs,
no pooled rescue, no B0 retraining (B0 is reused from hash-verified Task 7
evidence), no outcome-selected reruns.
...
(C5-A block policy wherever C5-A is a member, exactly as in Task 12), so
file scores are batching- and order-independent. Training masks use
``masking_seed = global step index`` over a deterministically shuffled
...
METRIC_CODE_FILES = (
    ...
    "experiments/sprint17_task20_k.py",
)
...
ARM_FILE_PREFIX = {"K1": "k1", "K2": "k2", "K3": "k3",
                   "K4": "k4", "K5": "k5", "K6": "k6",
                   "K7": "k7", "K8": "k8", "K9": "k9"}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 20 (Development only)")
...
def masking_policy_for(k_id: str):
    """Collate masking policy for a K arm (None = frozen B0 composition)."""
    if "C5-A" in K_MEMBERS[k_id]:
        from representation.sprint17_c5 import c5a_block_policy
        return c5a_block_policy
    return None


def k_config_dict(k_id: str, model_seed: int) -> dict:
    """Frozen per-K configuration: B0 values plus the union of member changes.

    Each present member contributes exactly the config delta its single-arm
    driver records (C2: stride/grids/support_weights; C3: local_encoder(+params);
    C5: masking_policy/criterion/horizons; C6: pooling(+params)); C7/C8/C9
    contribute no train-config keys (train-free scoring members). ``arm_id``
    names the K arm and ``adapter_description`` composes the member
    descriptions with no additional tuning. Optimizer, schedule, batch rule,
    precision, and budget are identical by construction.
    """
    ...
    if k_id not in K_ARMS:
        raise ValueError(f"Tasks 20-21 run only {K_ARMS}, got {k_id!r}")
    ...
    cfg["adapter_description"] = (
        f"{k_id} ({'+'.join(members)}): " + " | ".join(descs))
    return cfg
...
def build_model(cfg_dict: dict, device):
    ...
    members = K_MEMBERS[cfg_dict["arm_id"]]
    cfg = V1Config(
    ...
    # Patchifier: the C2-A overlap-explicit path wherever C2-A is a member
    # (bitwise B0 lattice parity control); B0 patchification otherwise.
    if "C2-A" in members:
        patchifier = arm_patchifier("C2-A")
        if (patchifier.cfg.patch_size != cfg.patch_size
                or patchifier.cfg.stride != cfg.stride):
            raise ValueError("C2-A patchifier geometry incompatible with K config")
    else:
        patchifier = Patchifier(PatchConfig(patch_size=cfg.patch_size,
                                            stride=cfg.stride,
                                            pad_end=cfg_dict["pad_end"]))
    # Local encoder: the C3-A residual dilated path wherever C3-A is a member.
    patch_encoder = None
    if "C3-A" in members:
        patch_encoder = arm_local_encoder(
            "C3-A", cfg.n_channels, cfg.d_model, cfg.dropout)
        if count_local_encoder_params(patch_encoder) != ARM_LOCAL_ENCODER_PARAMS["C3-A"]:
            raise ValueError("C3-A local-encoder parameter identity mismatch")
    # Pooling: the C6-A mean-plus-std path wherever C6-A is a member.
    if "C6-A" in members:
        pooling = arm_pooling("C6-A", cfg.d_model)
        if count_pooling_params(pooling) != ARM_POOLING_PARAMS["C6-A"]:
            raise ValueError("C6-A pooling parameter identity mismatch")
        model = C6ArmModel(cfg, arm_id="C6-A", pooling=pooling,
...
def train_seed(k_id: str, model_seed: int, fit_pool: list, device, log: list,
               steps: int, batch_size: int):
    """Train one arm seed for exactly ``steps`` optimizer steps (no selection)."""
    import torch
    import numpy as np
    ...
    cfg_dict = k_config_dict(k_id, model_seed)
    ...
    if "C5-A" in K_MEMBERS[k_id]:
        from representation.sprint17_c5 import arm_criterion
        criterion = arm_criterion(
            "C5-A", temperature=cfg_dict["contrastive_temperature"],
            prediction_weight=cfg_dict["prediction_weight"],
            lambda_max=cfg_dict["contrastive_weight_max"],
            ramp_steps=cfg_dict["contrastive_ramp_steps"],
            warmup_steps=cfg_dict["contrastive_warmup_steps"])
        trainer = RepresentationTrainer(model, criterion=criterion, optimizer=optimizer,
                                        scheduler=scheduler,
    ...
    while step < steps:
        ...
        for start in range(0, n_pool, batch_size):
            ...
            batches.append(collate_variable_files(
                ...
                masking_policy=masking_policy_for(k_id)))
            step += 1
...
def reload_b0(model_seed: int, ckpt_dir, device):
...
def c8_file_scores(lat_list: list[dict],
                   standardizer=None) -> tuple:
    """C8-A file scores + per-patch Huber energies via the frozen scorer module."""
    import numpy as np
    import torch
    ...
    for row in lat_list:
        ...
        energies = arm_patch_energies("C8-A", pred, targ, mask, standardizer)
...
def k6_aggregate(lat_list: list[dict], standardizer) -> tuple:
    """K6 file scores: C9-B window aggregation over C8-A Huber energies."""
    import numpy as np
    from representation.sprint17_c9 import arm_file_scores

    scores, patch, prov = [], [], []
    _, c8_patch = c8_file_scores(lat_list, standardizer)
    for row, e in zip(lat_list, c8_patch):
        ...
        if not np.isfinite(score):
            raise ValueError("non-finite K6 file score")
...
def main() -> int:
    ...
    ap.add_argument("--k", required=True, choices=list(K_ARMS),
                    help="frozen K combination to execute (one per invocation)")
    ap.add_argument("--b0-checkpoints", default="",
                    help="hash-verified B0 step-300 checkpoints (required for K5/K6)")
    ap.add_argument("--b0-caches", default="",
                    help="hash-verified B0 Development caches (required for K5/K6)")
    ap.add_argument("--b0-metrics", default="",
                    help="hash-verified Task 7 b0_per_seed_metrics.json for "
                         "paired deltas (required unless --smoke without deltas)")
    ...
    freeze = check_task19_freeze()
    members = K_MEMBERS[args.k]
    ...
    if args.b0_metrics:
        ...
        raise ValueError("--b0-metrics is required for non-smoke runs")
    ...
    # Lattice identity: the K patchifier (C2-A overlap-explicit path wherever
    # C2-A is a member, B0 patchification otherwise) must reproduce the
    # manifest patch count exactly (fail-closed parity check).
    if "C2-A" in members:
        from representation.sprint17_c2 import arm_patchifier as _arm_patchifier
        _k_patchifier = _arm_patchifier("C2-A")
    else:
        from synth.config import PatchConfig as _PatchConfig
        from synth.patchify import Patchifier as _Patchifier
        _k_patchifier = _Patchifier(_PatchConfig(patch_size=32, stride=16,
                                                  pad_end=True))
    arm_fit_patches = sum(_k_patchifier.patchify(s).N for s in fit_pool)
    if not args.smoke and arm_fit_patches != fit_patches:
        raise ValueError(
            f"{arm_id} lattice breach: arm patches {arm_fit_patches} != "
    ...
    # ---- Per-seed train-free path (K5/K6: B0 reload + combined rescoring) ----
    if arm_id in TRAIN_FREE_K and (not args.b0_checkpoints or not args.b0_caches):
        raise ValueError("--b0-checkpoints and --b0-caches are required for K5/K6")
    ckpt_dir_arg = Path(args.b0_checkpoints) if args.b0_checkpoints else None
    b0_cache_dir = Path(args.b0_caches) if args.b0_caches else None
    for model_seed in (seeds if arm_id in TRAIN_FREE_K else ()):
        st = time.time()
        ...
        # Fidelity proof: B0-MSE S_pred and bank S_pop from these latents
        # must reproduce the hash-verified caches (fail closed above atol).
        ...
        # C8-A standardizer fit on Fit forward residuals (both K5/K6 contain
        # C8-A; abnormal labels rejected inside fit).
        ...
        if arm_id == "K5":
            # K5: C8-A Huber S_pred + C7-A robust-shrinkage S_pop.
            from representation.sprint17_c7 import arm_reference
            cal_pred_k, _ = c8_file_scores(cal_lat, standardizer)
            fit_pred_k, _ = c8_file_scores(fit_lat, standardizer)
            ...
            cal_robots, cal_programs = condition_lists(cal_items)
            cal_pop_k, _ = reference.score(cal_emb, cal_robots, cal_programs)
            thr_pred = P.select_threshold(cal_pred_k)
            thr_pop = P.select_threshold(
                np.asarray(cal_pop_k, dtype=np.float64))
            ...
            for role in dev_roles:
                ...
                dev_out[role] = {"ids": ids, "S_pred": pred_k,
                                 "S_pop": np.asarray(pop_k, dtype=np.float64),
                                 "emb": demb, "t": ttime}
            bank_note = ("B0 checkpoint-restored (fidelity-proven) + C7-A "
                         "robust-shrinkage reference fit on Fit forward "
                         "embeddings + C8-A standardizer fit on Fit residuals")
            bank_rows = int(b0_bank.embeddings.shape[0])
        elif arm_id == "K8":
            # K8: C9-B windows over C8-A energies (S_pred) plus C7-A
            # robust-shrinkage S_pop — the K6 S_pred half and the K5 S_pop
            # half on the same reloaded B0 states; fresh q95 both branches.
            from representation.sprint17_c7 import arm_reference
            ...
            thr_pred = P.select_threshold(cal_pred_k)
            fit_robots, fit_programs = condition_lists(fit_items)
            fit_labels_k8 = [sample.file_label for _, sample in fit_items]
            fit_emb = torch.stack([r["file_embedding"] for r in fit_lat])
            reference = arm_reference("C7-A").fit(
                fit_emb, fit_robots, fit_programs, labels=fit_labels_k8)
            cal_emb = torch.stack([r["file_embedding"] for r in cal_lat])
            cal_robots, cal_programs = condition_lists(cal_items)
            cal_pop_k, _ = reference.score(cal_emb, cal_robots, cal_programs)
            thr_pop = P.select_threshold(
            ...
            for role in dev_roles:
                ...
                d_emb_fwd = torch.stack([r["file_embedding"] for r in rows])
                pop_k, _ = reference.score(d_emb_fwd, d_robots, d_programs)
                pred_k, patch_k, win_prov = k6_aggregate(rows, standardizer)
            ...
            bank_note = ("B0 checkpoint-restored (fidelity-proven) + C7-A "
                         "robust-shrinkage reference fit on Fit forward "
                         "embeddings + C8-A standardizer fit on Fit residuals + "
                         "C9-B window aggregation over C8-A energies")
            bank_rows = int(b0_bank.embeddings.shape[0])
        else:
            cal_pred_k, _, cal_win = k6_aggregate(cal_lat, standardizer)
            fit_pred_k, _, fit_win = k6_aggregate(fit_lat, standardizer)
            ...
            thr_pop = b0_thr_pop
            fit_emb = torch.stack([r["file_embedding"] for r in fit_lat])
            fit_pop = b0_bank.score(fit_emb).cpu().numpy().astype(np.float64)
            dev_out = {}
            win_spans = []
            for role in dev_roles:
                z = b0_dev[role]
            ...
            bank_note = ("B0 checkpoint-restored (fidelity-proven; S_pop reuse) + "
                         "C8-A standardizer fit on Fit residuals + C9-B "
                         "window aggregation over C8-A energies")
            bank_rows = int(b0_bank.embeddings.shape[0])
        ...
        seed_records[model_seed] = {
            ...
            "bank_rows": bank_rows, "bank_k": cfg.knn_k,
            "bank_source": bank_note,
            "scorer_provenance": scorer_prov,
            "fidelity_max_abs_diff": fidelity,
            "window_covered_span_mean": (
                float(np.mean(win_spans)) if arm_id in ("K6", "K8") and win_spans else None),
            ...
            "dev_out": dev_out, "fit_pred": fit_pred_k, "fit_pop": fit_pop,
            "cfg_dict": cfg_dict,
        }
    for model_seed in (seeds if arm_id in TRAINABLE_K else ()):
        st = time.time()
        model, trainer, cfg, patchifier, cfg_dict, first_loss, last = train_seed(
            arm_id, model_seed, fit_pool, device, run_log, steps, batch_size)
        params = count_parameters(model)
        flops = count_flops_reference(model)
        mpol = masking_policy_for(arm_id)
        ...
        cal_pred, cal_pop, cal_emb, _ = score_items(
            model, bank, patchifier, cfg, cal_items, device,
            masking_policy=mpol)
        ...
        for role in dev_roles:
            ...
            dev_out[role] = {"ids": [fid for fid, _ in items], "S_pred": pred,
                             "S_pop": pop, "emb": demb, "t": ttime}
        # K4 only: rescore S_pop with the C7-A robust-shrinkage reference
        # fit on the K4 Fit embeddings (Task 14 flow on K4-trained states).
        # S_pred stays on the trained K4 path with fresh q95 thresholds.
        if arm_id == "K4":
            from representation.sprint17_c7 import arm_reference
            fit_robots, fit_programs = condition_lists(fit_items)
            ...
            cal_robots, cal_programs = condition_lists(cal_items)
            cal_pop_k4, _ = reference.score(cal_emb, cal_robots, cal_programs)
            thr_pop = P.select_threshold(
                np.asarray(cal_pop_k4, dtype=np.float64))
            fit_pop_k4, _ = reference.score(
                fit_emb, fit_robots, fit_programs)
            fit_pop = np.asarray(fit_pop_k4, dtype=np.float64)
            for role in dev_roles:
                d_ids = dev_out[role]["ids"]
                d_robots, d_programs = condition_lists(
                    [(fid, dev_by_id[role][fid]) for fid in d_ids])
                pop_k4, _ = reference.score(
                    dev_out[role]["emb"], d_robots, d_programs)
                dev_out[role]["S_pop"] = np.asarray(pop_k4, dtype=np.float64)
            if not (np.isfinite(thr_pop)
                    and np.isfinite(fit_pop).all()
                    and all(np.isfinite(dev_out[r]["S_pop"]).all()
                            for r in dev_roles)):
                raise ValueError(f"non-finite K4 C7-A S_pop: seed {model_seed}")
        # K9 only: rescore S_pred with C8-A+C9-B and S_pop with C7-A on the
        # K9 forward latents (block masks: K9 contains C5-A, exactly as in
        # Task 12). The trained upstream stack (C2+C3+C5+C6) is untouched.
        k9_win_spans: list = []
        if arm_id == "K9":
            from representation.sprint17_c7 import arm_reference
            from representation.sprint17_c8 import HuberStandardizer
            mpol_k9 = masking_policy_for(arm_id)
            k9_fit_lat = latent_items(
                model, patchifier, cfg, fit_items, device,
                masking_policy=mpol_k9)
            k9_cal_lat = latent_items(
                model, patchifier, cfg, cal_items, device,
                masking_policy=mpol_k9)
            k9_fit_res = torch.cat(
                [(r["predicted"] - r["target"])[r["prediction_mask"]]
                 for r in k9_fit_lat], dim=0)
            k9_fit_labels = []
            for r, (_, sample) in zip(k9_fit_lat, fit_items):
                n = int(r["prediction_mask"].sum().item())
                k9_fit_labels += [sample.file_label] * n
            k9_standardizer = HuberStandardizer().fit(
                k9_fit_res, labels=k9_fit_labels)
            k9_scorer_prov = k9_standardizer.provenance()
            k9_cal_pred, _, _ = k6_aggregate(k9_cal_lat, k9_standardizer)
            k9_fit_pred, _, _ = k6_aggregate(k9_fit_lat, k9_standardizer)
            thr_pred = P.select_threshold(k9_cal_pred)
            ...
            k9_reference = arm_reference("C7-A").fit(
                k9_fit_emb, k9_fit_robots, k9_fit_programs,
                labels=k9_fit_labels_c7)
            k9_cal_emb = torch.stack([r["file_embedding"] for r in k9_cal_lat])
            k9_cal_robots, k9_cal_programs = condition_lists(cal_items)
            k9_cal_pop, _ = k9_reference.score(
                k9_cal_emb, k9_cal_robots, k9_cal_programs)
            thr_pop = P.select_threshold(
                np.asarray(k9_cal_pop, dtype=np.float64))
            k9_fit_pop, _ = k9_reference.score(
                k9_fit_emb, k9_fit_robots, k9_fit_programs)
            fit_pred = k9_fit_pred
            fit_pop = np.asarray(k9_fit_pop, dtype=np.float64)
            for role in dev_roles:
                k9_items = [(r["file_id"], dev_by_id[role][r["file_id"]])
                            for r in dev_rows[role]
                            if r["file_id"] in dev_by_id[role]]
                k9_dev_lat = latent_items(
                    model, patchifier, cfg, k9_items, device,
                    masking_policy=mpol_k9)
                k9_ids = [r["file_id"] for r in k9_dev_lat]
                k9_pred, k9_patch, k9_prov = k6_aggregate(
                    k9_dev_lat, k9_standardizer)
                k9_win_spans += [w.get("covered_span", 0) for w in k9_prov]
                k9_d_robots, k9_d_programs = condition_lists(
                    [(fid, dev_by_id[role][fid]) for fid in k9_ids])
                k9_d_emb = torch.stack(
                    [r["file_embedding"] for r in k9_dev_lat])
                k9_pop, _ = k9_reference.score(
                    k9_d_emb, k9_d_robots, k9_d_programs)
                k9_ttime = [patchifier.patch_to_timestep_scores(
                                k9_patch[i].astype(np.float32),
                                k9_dev_lat[i]["starts"],
                                k9_dev_lat[i]["valid_len"],
                                k9_dev_lat[i]["file_len"])
                            for i in range(len(k9_ids))]
                k9_ttime = [np.asarray(t, dtype=np.float64) for t in k9_ttime]
                dev_out[role] = {"ids": k9_ids, "S_pred": k9_pred,
                                 "S_pop": np.asarray(k9_pop, dtype=np.float64),
                                 "emb": k9_d_emb, "t": k9_ttime}
            if not (np.isfinite(thr_pred) and np.isfinite(thr_pop)
                    and np.isfinite(fit_pred).all()
                    and np.isfinite(fit_pop).all()
                    and all(np.isfinite(dev_out[r]["S_pred"]).all()
                            and np.isfinite(dev_out[r]["S_pop"]).all()
                            for r in dev_roles)):
                raise ValueError(f"non-finite K9 rescoring: seed {model_seed}")
        corr = float(np.corrcoef(
            np.concatenate([dev_out[r]["S_pred"] for r in dev_roles]),
            np.concatenate([dev_out[r]["S_pop"] for r in dev_roles]))[0, 1])
        if not np.isfinite(corr) or corr >= 0.999:
        ...
        ckpt_path = ckpt_dir / f"{prefix}_seed{model_seed}_step{trainer.step}.pt"
        ...
        if arm_id == "K9":
            seed_records[model_seed]["bank_source"] += (
                " + C8-A standardizer fit, C9-B window aggregation (S_pred), "
                "and C7-A reference fit (S_pop) on K9 block-masked forward "
                "latents")
            seed_records[model_seed]["window_covered_span_mean"] = (
                float(np.mean(k9_win_spans)) if k9_win_spans else None)
    ...
    (outdir / "run.log.json").write_text(json.dumps(to_jsonable({
        ...
        "task19_freeze_sha256": TASK19_FREEZE_SHA256,
        "seeds": list(seeds),
        "steps": steps,
...
def assemble_evidence(outdir: Path, arm_id: str, seeds: tuple, seed_records: dict,
    ...
    members = list(K_MEMBERS[arm_id])
    components = [m.split("-")[0] for m in members]
    prefix = ARM_FILE_PREFIX[arm_id]
    ...
    doc = {
        ...
        "config": {
            ...
            "adapter_description": seed_records[seeds[0]]["cfg_dict"][
                "adapter_description"],
            "optimizer_steps": OPTIMIZER_STEPS,
            "checkpoint_step": CHECKPOINT_STEP,
            "calibration_quantile": CALIBRATION_QUANTILE,
