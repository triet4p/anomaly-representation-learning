"""Sprint 17 Task 13 — train and evaluate C6 file-pooling arms on Development.

Runs exactly one registered C6 arm per invocation (``--arm C6-A`` or ``--arm
C6-B``) under the frozen B0 parity contract: same Fit/Calibration/Development
...
C6-A is valid-patch mean-plus-population-std pooling projected to the existing
file-embedding dimension (one Linear(256,128) adapter, 32,896 params); C6-B is
Ilse-style gated attention pooling restricted to valid patches (tanh/sigmoid
gates D->32 plus a 32->1 scorer, 8,289 params; no anomaly-derived input or
target). Only the context-latent-to-file-embedding function changes:
patchification, local encoder, context encoder, objective, projector,
geometry, scorers, aggregation, and metric code are unchanged.
...
C6_ARMS = ("C6-A", "C6-B")
ARM_FILE_PREFIX = {"C6-A": "c6a", "C6-B": "c6b"}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 13 (Development only)")
...
def arm_config_dict(arm_id: str, model_seed: int) -> dict:
    """Frozen per-arm configuration: B0 values plus the declared C6 change.

    Only ``arm_id``, ``pooling``, ``pooling_params``,
    ...
    from representation.sprint17_c6 import (
        ARM_ADAPTER_DESCRIPTION, ARM_POOLING_PARAMS, arm_pooling)
    if arm_id not in C6_ARMS:
        raise ValueError(f"Task 13 runs only {C6_ARMS}, got {arm_id!r}")
...
def build_model(cfg_dict: dict, device):
    ...
    # C6 keeps B0 patchification and encoding exactly; only file pooling is swapped.
    patchifier = Patchifier(PatchConfig(patch_size=cfg.patch_size,
                                        stride=cfg.stride,
                                        pad_end=cfg_dict["pad_end"]))
    pooling = arm_pooling(cfg_dict["arm_id"], cfg.d_model)
    if count_pooling_params(pooling) != ARM_POOLING_PARAMS[
            cfg_dict["arm_id"]]:
        raise ValueError("arm pooling parameter identity mismatch")
    model = C6ArmModel(cfg, arm_id=cfg_dict["arm_id"], pooling=pooling,
...
def main() -> int:
    ...
    ap.add_argument("--arm", required=True, choices=list(C6_ARMS),
                    help="registered C6 arm to execute (one per invocation)")
    ...
    # C6 keeps B0 patchification exactly: the arm patch count must equal the
    ...
    if not args.smoke and arm_fit_patches != fit_patches:
        raise ValueError(
            f"C6 lattice breach: arm patches {arm_fit_patches} != "
...
def assemble_evidence(outdir: Path, arm_id: str, seeds: tuple, seed_records: dict,
    ...
    from representation.sprint17_c6 import ARM_ADAPTER_DESCRIPTION
