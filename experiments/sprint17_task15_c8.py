"""Sprint 17 Task 15 — evaluate C8 scorer arms on Development (no training).

Runs exactly one registered C8 arm per invocation (``--arm C8-A`` or ``--arm
C8-B``) under the frozen B0 parity contract: same Fit/Calibration/Development
...
C8-A is non-query-conditioned standardized Huber prediction-residual energy
(Fit-fitted per-dimension median/MAD, delta 1.0); C8-B is non-query-conditioned
cosine distance between predicted and target latents. Only the per-patch
prediction-residual energy changes: masks, file-mean aggregation,
representations, targets, S_pop, calibration rule, support, localization
mapping, and metric code are unchanged, and S_pop is reused bitwise from the
hash-verified B0 caches.
...
against the hash-verified B0 caches (fidelity proof), S_pop is reused bitwise
from those caches, and S_pred is rescored by the C8 energy.
"""
...
C8_ARMS = ("C8-A", "C8-B")
ARM_FILE_PREFIX = {"C8-A": "c8a", "C8-B": "c8b"}
...
def load_verified_root(data_root: Path, group: str, role: str,
    ...
    if group in ("CONFIRMATION", "SEALED"):
        raise ValueError(f"refusing {group} access in Task 15 (Development only)")
...
def arm_config_dict(arm_id: str, model_seed: int) -> dict:
    """Frozen per-arm configuration: B0 values plus the declared C8 change.

    Only ``arm_id``, ``scorer``, and ``adapter_description`` differ
    from :func:`b0_config_dict`. Masks, aggregation, representations,
    targets, S_pop path, objective, optimizer, schedule, batch rule,
    precision, and budget are identical by construction; this driver
    executes zero optimizer steps (representations inherit the frozen B0
    step-300 states).
    """
    from representation.sprint17_c8 import ARM_ADAPTER_DESCRIPTION
    if arm_id not in C8_ARMS:
        raise ValueError(f"Task 15 runs only {C8_ARMS}, got {arm_id!r}")
...
def count_flops_reference(model, t: int = FLOP_REFERENCE_T) -> int:
    ...
    try:
        ...
        # C8 keeps B0 patchification and encoding: the reference batch uses the
...
def build_model(cfg_dict: dict, device):
    ...
    # C8 keeps the B0 graph exactly (frozen representations); only the
    # S_pred per-patch energy is replaced downstream of the forward.
    ...
    if count_parameters(model) != B0_PARAMS:
        raise ValueError("C8 model graph is not the frozen B0 graph")
    if count_flops_reference(model) != B0_FLOPS_REFERENCE:
        raise ValueError("C8 scoring FLOPs differ from the B0 reference")
    model.to(device)
...
def main() -> int:
    ...
    ap.add_argument("--arm", required=True, choices=list(C8_ARMS),
                    help="registered C8 arm to execute (one per invocation)")
    ...
    # C8 keeps B0 patchification exactly: the arm patch count must equal the
    ...
    if not args.smoke and arm_fit_patches != fit_patches:
        raise ValueError(
            f"C8 lattice breach: arm patches {arm_fit_patches} != "
    ...
    # ---- Per-seed frozen reload / latents / C8 fit / calibrate / score ----
    ...
    for model_seed in seeds:
        ...
        # 5. Fit the C8-A standardizer on Fit forward residuals (Fit-only
        # healthy rows; abnormal labels rejected inside fit). C8-B fits nothing.
        standardizer = None
        ...
        # 6. C8 S_pred on forward latents (Calibration for thresholds,
        # Development for metrics, Fit for the stability denominator).
        cal_pred_c8, _ = c8_file_scores(cal_lat, arm_id, standardizer)
        fit_pred_c8, _ = c8_file_scores(fit_lat, arm_id, standardizer)
        # 7. Thresholds: S_pred fresh q95 on Calibration C8 scores; S_pop
        # reused bitwise from B0 (proof: recomputed q95 on forward bank S_pop
        # matches outside smoke).
        thr_pred = P.select_threshold(cal_pred_c8)
        ...
        for role in dev_roles:
            ...
            ttime = [patchifier.patch_to_timestep_scores(
                         patch_c8[i].astype(np.float32), rows[i]["starts"],
            ...
            dev_out[role] = {"ids": ids, "S_pred": pred_c8,
        ...
        # Persist C8 caches (same contract/keys as B0; S_pop/emb reused bitwise,
        # timestep scores recomputed from C8 patch energies).
        ...
        seed_records[model_seed] = {
            ...
            "scorer_kind": {"C8-A": "HuberStandardizer+huber-mean",
                            "C8-B": "cosine-distance-mean"}[arm_id],
            ...
            "dev_out": dev_out, "fit_pred": fit_pred_c8, "fit_pop": fit_pop_b0,
            "cfg_dict": cfg_dict,
        }
