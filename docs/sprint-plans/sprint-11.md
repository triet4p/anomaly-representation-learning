...
- [ ] **Task 24 — Git-synchronized server preflight.** Commit and push the reviewed source, inspect `trietlm@192.168.30.244` read-only, preserve unrelated remote work, fast-forward pull or use a safe clean checkout, and record exact local/remote commit parity, repo path, runtime, disk, CPU/GPU, and explicit data/output roots. Use PowerShell OpenSSH with `BatchMode=yes`; never reset, clean, force-push, scan mounts, or persist credentials. Evidence: `artifacts/sprint-11/task-24.md`.
- [ ] **Task 25 — Full chronological dataset generation on server.** At the verified commit, generate the configured 3–6 month server dataset with explicit roots and the locked seed/profile. Keep shards and bulk schedules server-side and uncommitted; bring back only manifests, logs, checksums, counts, and bounded diagnostics. Evidence: `artifacts/sprint-11/task-25.md`.
- [ ] **Task 26 — Server dataset evidence audit.** Verify persisted server data through the public reader: schedule non-overlap, shared routes, asynchronous utilization, health/failure/maintenance causality, program sensitivity, anomaly/precursor prevalence, quarantine, exact static/temporal split membership, reload integrity, and deterministic manifest provenance. Correct generator defects and regenerate before passing. Evidence: `artifacts/sprint-11/task-26.md`.
