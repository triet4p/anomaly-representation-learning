...
- [x] **Task 29 — Coefficient and architecture selection gate.** Mechanical predeclared gate selected exactly one hybrid (`balance-b`; `balance-a`/`balance-c` rejected on separation/ordering, control as valid reference) using only training/healthy-validation/synthetic diagnostics. Full-run coefficients frozen to `full_hybrid.yaml` (sha256 `cdf73751…`) at `86812d3`; recorded prerequisite: Batch E3 must commit an explicit full-stage schedule (default warmup 500 would leave alpha at 0 over ~300 steps) before Tasks 30+. Evidence: `artifacts/sprint-11/task-29.md`.
