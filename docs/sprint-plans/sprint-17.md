...
**Status:** ACTIVE — The user explicitly resumed Sprint 17 for Batch B only. Tasks 1–6 remain accepted with Batch A PASS and zero actionable findings. Task 4 preserves the original protocol-v3 `BLOCKED / NOT_MEASURABLE` result while protocol v4 applies only the exact one-cell post-observation waiver, yielding qualified `MEASURABLE_WITH_USER_WAIVER`. Execute Tasks 7–9 through a zero-actionable Batch B evidence review, then pause before Batch C / Task 10. Sprint 15 Sealed H-SEAL-37..40 and Confirmation remain prohibited.
...
- [x] **Task 8 — Audit baseline reproducibility and support.** Confirm deterministic config identity, seed-complete runs, finite outputs, common event support, score separation, parameter/compute accounting, and measurable oracle ceiling. Evidence: `artifacts/sprint-17/task-8.md`. (Done: 60/60 mechanical checks PASS plus CPU checkpoint-reload rescore PASS; oracle ceiling 0.7981 vs B0 0.6720; Task 9 review and Batch C remain pending.)
...
- [ ] **Task 14 — Execute C7 geometry/bank alternatives.** Compare `C7-A` and `C7-B` individually using only Fit healthy-reference rows and explicit bank row/source provenance; keep `S_pop` separate. Evidence: `artifacts/sprint-17/task-14.md`.
- [ ] **Task 15 — Execute C8 scorer alternatives.** Compare `C8-A` and `C8-B` individually with frozen representations/targets and keep `S_pred` separate from `S_pop`. Evidence: `artifacts/sprint-17/task-15.md`.
- [ ] **Task 16 — Execute C9 aggregation alternatives.** Compare `C9-A` and `C9-B` individually over identical valid patch scores and preserve localization/duration accounting. Evidence: `artifacts/sprint-17/task-16.md`.
- [ ] **Task 17 — Select one Development arm per suspect.** Apply only the frozen deterministic rule, publish every paired single-arm result and invalid arm, and freeze the selected arm IDs without opening Confirmation. Evidence: `artifacts/sprint-17/task-17.md`.
- [ ] **Task 18 — Pass Batch C evidence review.** Verify all 14 individual comparisons, one-component isolation, complete negative results, selection mechanics, and zero actionable findings. Evidence: `artifacts/sprint-17/review-batch-c.md`.

### Batch D — Predeclared interactions on Development

- [ ] **Task 19 — Freeze selected combination configs.** Materialize K1–K9 configs from the exact selected single-arm implementations; no combo-specific hyperparameter change or outcome-selected omission. Evidence: `artifacts/sprint-17/task-19.md`.
...
- The reviewed Task 6 gate is complete: Batch A PASS with zero actionable findings is recorded at `artifacts/sprint-17/review-batch-a.md`, and the explicit user pre-GPU stop is reached. No GPU connection, model run, Task 7 artifact, or later-task artifact occurred; Task 7/GPU and Sealed access remain pending/prohibited, and resume requires new explicit user instruction.
