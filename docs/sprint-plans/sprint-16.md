...
**Status:** ACTIVE — Tasks 1–17 passed evidence review with 0 actionable findings. Task 18's original four-PASS, containment PASS, R2 scoped PASS, and R3 scoped PASS remain recorded with explicit never-collapsed lineage. Deep R4 was FAIL with two LOW findings; its correction scoped review `history://Sprint16R4CorrectionEvidence:73-75` was PASS with zero actionable findings. Updated-provider Deep R5 then returned FAIL (confidence 0.93) with one actionable LOW, F-DR16R5-01; its correction scoped review `history://Sprint16R5CorrectionEvidence:59-61` was PASS with zero actionable findings. Deep R6 then returned FAIL (confidence 0.88) with two actionable LOW findings: F-DR16R6-01 (two unpinned R2 scoped-PASS locators in Task 17) and F-DR16R6-02 (reactivated bare correction-review URIs). R6 correction scoped review `history://Sprint16R6CorrectionEvidence:93-99` is PASS with zero actionable findings and closes both R6 findings. The next required gate is a fresh sprint-wide deep review under the updated deep-reviewer provider. Accepted attribution state remains `UNRESOLVED` with empty/not-authorized Sprint 17 scope (robust holdings: P+W-macro unavailable so R-labels provisional; zero R3 paths; §9.1 deviation recorded). Sprint 16 remains ACTIVE and Task 19 remains `[~]`, not final; no task-19 artifacts yet. Sealed H-SEAL-37..40 remain forbidden to open/score/probe.
...
- [x] **Task 17 — Define the bounded Sprint 17 recovery scope.** Convert only causally supported bottlenecks into proposed implementation tasks and gates. Do not prescribe a wholesale architecture rewrite when attribution is unresolved, and do not include calibrated-risk work before useful event ranking. Evidence: `artifacts/sprint-16/task-17.md`.
- [x] **Task 18 — Pass batch evidence gates.** Review each completed batch with an evidence reviewer. Any actionable defect returns to the same implementation owner and the affected gate repeats. Preserve all failed, corrected, and superseded records. Evidence: `artifacts/sprint-16/review-*.md`.
