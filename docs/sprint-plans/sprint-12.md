...
- [x] **Task 3 — Compare handcrafted and frozen-latent conditional baselines.** Implement an interpretable valid-patch feature set covering channel level/RMS/variance/slope, differences/cross-channel relationships, spectral bands, transitions, and duration. Compare handcrafted features and accepted frozen learned features using the same regularized hierarchical healthy-only geometry and fixed upper-tail file aggregation. Retain localization, variable-length masks, sparse fallback, and score provenance. Run both variants on the historical Sprint 11 diagnostic benchmark, report discrimination/localization/false alerts and group slices, and distinguish absent signal from an inadequate baseline. Evidence: `artifacts/sprint-12/task-3.md`. (CORRECTED: standard AUROC + frozen dev-train standardization; AUROC 0.663 vs 0.509–0.514 — handcrafted modest, learned fails; awaiting re-review.)
- [x] **Task 4 — Audit signal visibility through actual preprocessing.** Execute paired observable-signal diagnostics before and after the production preprocessing path, including amplitude and cross-channel changes and benign nuisance controls. Trace whether anomaly information is removed by preprocessing, feature extraction, contextual encoding, or scoring; quantify rather than infer from source alone. Use labels/masks only for post-hoc diagnostics. Evidence: `artifacts/sprint-12/task-4.md`. (CORRECTED: calibrated null, family-wise two-sided amplitude, same centered S0/S1 statistic, conditional S2/S3, comparable nuisance; loss stage NOT identified; awaiting re-review.)
...
- Batch B: awaiting re-review (CORRECTED reruns after 6 actionable findings in artifacts/sprint-12/review-B.md: standard sklearn AUROC everywhere with ties/perfect/reversed/single-class regression (prior inverted numbers superseded); handcrafted standardized with dev-train-only frozen transform under SAME hierarchy/shrinkage/aggregation — AUROC 0.663 vs learned 0.509–0.514 ("handcrafted modest, learned fails" cell); cross-channel calibrated null (effect +0.34, empirical-p median 0.44 — no raw-information claim); amplitude by family two-sided; S0/S1 same centered statistic with truthful slicing/padding account; conditional S2/S3 hierarchy; comparable nuisance nulls; exact loss stage NOT claimed; A3 uncertainty preserved; worker does not self-accept).
- Batch C: pending.
- Batch D: pending.
